class Admin::PrototypesController < ApplicationController
  ...
  def create
    ...
    if @prototype.save
      redirect_to admin_prototypes_path, notice: 'Prototype was successfully created.'
    else
      render action: "new"
    end
  end
  ...
  def update
    if @prototype.update_attributes(params[:prototype])
      redirect_to admin_prototypes_path, notice: 'Prototype was successfully updated.'
    else
      render action: "edit"
    end
  end
